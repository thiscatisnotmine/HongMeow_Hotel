      ...
      .then(data => {
          ...
          data.forEach(check => {
              ...
              row.innerHTML = `
                  ...
                  <td>
                    <button class="blue-btn" onclick='checkout("${check.BID}","${check.RTID}",${check.RID})'>
                        Check-Out
                    </button>
                  </td> 
              `;
...
// check-out
async function checkout(bookingId, roomCode, roomNumber) {
  try {
    const response = await fetch(`${api}/bookedroom/update-status`, { // อัปเดตสถานะห้องเป็น check-out ในตาราง bookedroom
      method: 'PUT', 
      headers: {
        'Content-Type': 'application/json'
      },
